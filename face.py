...
def build_fr_model(input_shape):
    resent_model = InceptionResNetV2(
...
def load_images(data_dir, image_paths, image_shape):
    ...
    for i, image_path in enumerate(image_paths):
        ...
        try:
            # Load image
            loaded_image = image.load_img(os.path.join(
                data_dir, image_path), target_size=image_shape)
...
if __name__ == '__main__':
    ...
    discriminator.compile(
        loss=['binary_crossentropy'], optimizer=dis_optimizer)
    ...
    adversarial_model = Model(
        inputs=[input_z_noise, input_label], outputs=[valid])
    adversarial_model.compile(
        loss=['binary_crossentropy'], optimizer=gen_optimizer)
    ...
    loaded_images = load_images(
        wiki_dir, images[:1000], (image_shape[0], image_shape[1]))
    ...
    if TRAIN_GAN:
        for epoch in range(epochs):
            ...
            for index in range(number_of_batches):
                ...
                images_batch = loaded_images[index *
                ...
                g_loss = adversarial_model.train_on_batch(
                    [z_noise2, random_labels], np.ones((batch_size, 1)))
            ...
            write_log(tensorboard_callback, 'g_loss',
                      np.mean(gen_losses), epoch)
            write_log(tensorboard_callback, 'd_loss',
                      np.mean(dis_losses), epoch)

            print("Epoch {} completed in {:.2f} seconds | g_loss: {:.5f} | d_loss: {:.5f}".format(
                epoch + 1, time.time() - start_time, np.mean(gen_losses), np.mean(dis_losses)))
            ...
            if (epoch+1) % 10 == 0:
                ...
                for i, img in enumerate(gen_images[:5]):
                    save_rgb_img(
                        img, path="./results/img_{}_{}.png".format(epoch, i))
    ...
    if TRAIN_ENCODER:
        ...
        for epoch in range(epochs):
            ...
            write_log(tensorboard, "encoder_loss",
                      np.mean(encoder_losses), epoch)
    ...
    if TRAIN_GAN_WITH_FR:
        ...
        resized_images = Lambda(lambda x: K.resize_images(
            x, height_factor=3, width_factor=3, data_format='channels_last'))(gen_images)

        embeddings = fr_model(resized_images)

        # Create a Keras model and specify the inputs and outputs for the network
        fr_adversarial_model = Model(
            inputs=[input_image, input_label], outputs=[embeddings])

        # Compile the model
        fr_adversarial_model.compile(
            loss=euclidean_distance_loss, optimizer=adversarial_optimizer)
        ...
        for epoch in range(epochs):
            ...
            if (epoch+1) % 10 == 0:
                ...
                for i, img in enumerate(gen_images[:5]):
                    save_rgb_img(
                        img, path="./results/img_opt_{}_{}.png".format(epoch, i))
